...
db=Redis.new
tuple_name="window1"
p $temp_config=yaml_dbr("house1","./bin_ac/cgi-bin/config/house1")
...
#p change
    ...
    if change != now
      ...
      begin
        p change
        if now[0] and (!now[1])  
          db.rpush(:numato,"relay off 8")
          sleep 2
          db.rpush(:numato,"relay on 7")
          sleep 2
        elsif now[1] and (!now[0])
          db.rpush(:numato,"relay off 7")
          sleep 2
          db.rpush(:numato,"relay on 8")
          sleep 2
        else
          db.rpush(:numato,"relay off 7")
          sleep 2
