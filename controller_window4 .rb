...
db=Redis.new
tuple_name="window3"
p $temp_config=yaml_dbr("house3","./bin_ac/cgi-bin/config/house3")
  ...
  loop do
    ...
    if min_before!=t.min && t.sec%10<8
      ...
      begin
        ...
        data=db.get("usbrh1:degree")
        if data=="0"
          p "usbrh:degree"
          data=db.get("usbrh:degree")
        end
        temp=data.to_f
...
#p change
    ...
    if change != now
      ...
      begin
        p change
        if now[0] and (!now[1])  
          db.rpush(:numato,"relay off C")
          sleep 2
          db.rpush(:numato,"relay on B")
          sleep 2
        elsif now[1] and (!now[0])
          db.rpush(:numato,"relay off B")
          sleep 2
          db.rpush(:numato,"relay on C")
          sleep 2
        else
          db.rpush(:numato,"relay off B")
          sleep 2
