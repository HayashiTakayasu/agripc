...
db=Redis.new
tuple_name="window2"
p $temp_config=yaml_dbr("house2","./bin_ac/cgi-bin/config/house2")
...
#p change
    ...
    if change != now
      ...
      begin
        p change
         if now[0] and (!now[1])  
          db.rpush(:numato,"relay off A")
          sleep 2
          db.rpush(:numato,"relay on 9")
          sleep 2
        elsif now[1] and (!now[0])
          db.rpush(:numato,"relay off 9")
          sleep 2
          db.rpush(:numato,"relay on A")
          sleep 2
        else
          db.rpush(:numato,"relay off 9")
          sleep 2
