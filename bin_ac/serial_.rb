    ...
    class Serial
      ...
      def serial(command=@command,port=@port,speed=@speed,time_out=@time_out)
        ...
        begin
          ...
          #else
          #  p s
          #  raise "#{port} #{command.chomp} communicate failed."
          end
          return false
        end#timeout rescue
        @res=r
...
=begin
        begin
          p stop_bit=command[-1].chr
          timeout(timeout_sec+0.5) do
            #sleep 0.05
            s=open(port,"r+b")
            ##root only
            #RS232 binary,9600bps,8bit,stopbit=1(default),non-parity,timeout=1sec
            #echo back
            #port="/dev/ttyUSB0"
            #p send="stty raw -echo -parenb #{speed.to_s} cs8 time #{timeout_sec.to_s}0 <#{port}"
            p send="stty raw -echo -parenb #{speed.to_s} cs8 time 10<#{port}"
            system(send)
                 #p "stty raw -echo -parenb cstopb -hupcl inlcr #{speed.to_s} cs8 time #{timeout_sec.to_s}0 <#{port}"
            #system("stty raw -echo -parenb #{speed.to_s} cs8 time #{timeout_sec.to_s}0 <#{port}")
             
            s.write(command)#"%01#RDD0000000015**\r")
            r=s.gets(stop_bit)
