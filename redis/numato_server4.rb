...
p "manual initialize"
  while command=db.blpop(:numato,:time_out => 0.1)
      if command[1]
        ...
        p data=Time.now.strftime("%Y-%m-%d %H:%M:%S")+","+bit_str+","+command[1]
      
        str=%!sudo python ./namuto.py "#{command[1]}"!
        res=`#{str}`
  ...
  while command=db.blpop(:numato,:time_out => 0.1)
    begin
      ...
      if command[1]
        ...
        str=%!sudo ./ReadC1line /dev/ttyACM0 "#{command[1]}"!
        begin
          res0=`#{str}`
          res=res0.split("\n\r")[1]
          if command[1].include?("gpio read 0")
            if res=="1" 
              db.set(:rain,"1") 
            else
              db.set(:rain,"0")
    ...
    #  p data=Time.now.strftime("%Y-%m-%d %H:%M:%S")+","+command[1]
     # 
    #  p str=%!sudo python ./namuto.py "#{command[1]}"!
    #  p res=`#{str}`
    #
   #   Loger::loger("./log/db.txt",data)  
    end
